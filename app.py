...
load_dotenv()  # .env(로컬) 로드

# 우선순위: Streamlit Cloud Secrets > .env
API_KEY = None
ADMIN_CODE_SECRET = None
try:
    API_KEY = st.secrets.get("OPENAI_API_KEY", None)
    ADMIN_CODE_SECRET = st.secrets.get("ADMIN_CODE", None)
except Exception:
    pass
if not API_KEY:
...
if client:
    st.success("🔐 OpenAI API 키 감지됨 — 실제 피드백 생성 모드")
else:
    st.warning("🧪 OpenAI API 키가 없어 데모 모드로 동작합니다 (샘플 규칙엔진으로 피드백).")
...
def save_log(pid: str, trial: int, lang: str, topic: str, stage: str, text: str):
    ...
    with open("logs.csv", "a", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        # 헤더는 생략(분석 시 별도 지정), 원하는 경우 첫 생성 시 헤더 쓰기 가능
        w.writerow(row)
...
def load_prompt(path: str) -> str:
    try:
        ...
        return (
            ...
            "Use short polite sentences. Provide: Praise → 2 errors "
            "(explain & example) → 2 Bible examples → rewrite instruction → "
...
def lookup_examples(text: str, topk: int = 2) -> list[dict]:
    ...
    if len(uniq) < topk:
        rest = topk - len(uniq)
        samp = corpus.sample(n=min(rest, len(corpus))).to_dict(orient="records")
        uniq.extend(samp)
    return uniq[:topk]
...
if ADMIN_CODE_SECRET and admin_input == ADMIN_CODE_SECRET:
    ...
    # 로컬 개발 시 임시 허용(Cloud에서는 Secrets 사용 권장)
    is_admin = True

if is_admin:
    st.sidebar.success("교수자 모드 활성화 ✅")
    if os.path.exists("logs.csv"):
        st.sidebar.caption("현재 로그 미리보기 (최근 50줄)")
        try:
...
strategy = st.selectbox(
    "교수전략(시연용)",
    ["자동 선택", "모형 제시 (Modeling)", "단계 안내 (Scaffolding)", "확장 유도 (Extension)"],
    index=1,
)
...
def demo_feedback(text: str, examples_block: str, lang: str) -> str:
    ...
    if len(tips) < 2:
        tips.append(("[연결어]", "이유-결과를 '그래서/그러나' 등으로 잇습니다.", "예) 사랑을 배웠습니다. 그래서 감사했습니다."))
    if len(tips) < 2:
        tips.append(("[문장 분리]", "긴 문장은 두 문장으로 나눕니다.", "예) 수업이 끝났습니다. 곧 정리했습니다."))

    if lang == "영어 (EN)":
        lines = []
        lines.append("Great effort! Your faith and intention come through clearly.")
        lines.append(f"- [Particle] Use '의/을/를' correctly after nouns. e.g., 하나님의 사랑을 배웠습니다.")
        lines.append(f"- [Polite ending] Use '-습니다' for academic tone. e.g., 배웠습니다.")
        lines.append(examples_block.strip() or "📖 (No related Bible example)")
        lines.append("Please rewrite your paragraph in 3–5 sentences using the feedback.")
        lines.append("Strength: Clear topic | Next goal: Practice particles and polite endings.")
        return "\n".join(lines)

    # 한국어 + 이중언어 기본은 한국어 본문
    lines = []
    lines.append("좋은 시도예요. 신앙의 마음이 잘 느껴집니다.")
    ...
    if lang == "이중언어 (KR+EN)":
        ...
        lines.append("Focus on particles and polite endings (e.g., 하나님의 사랑을 배웠습니다). Rewrite in 3–5 sentences.")
    return "\n".join(lines)
...
if run_clicked:
    if not agree:
        ...
        # topic 강제 선택 시 해당 태그 포함 구절 우선
        if topic and topic != "(자동)":
            tagged = corpus[corpus["tags"].str.contains(re.escape(topic), na=False)]
            if not tagged.empty:
                ex_tag = tagged.sample(n=min(2, len(tagged))).to_dict(orient="records")
                if ex_tag: examples = ex_tag
        ...
        if language == "한국어 (KR)":
            ...
            # KR 프롬프트를 기반으로 하되, 마지막에 EN 요약 2–3줄 지시 추가
            base = (
                prompt_kr
...
"""
        ...
        if client:
            try:
                ...
                feedback = f"(오류) API 호출 실패: {e}\n\n" + demo_feedback(student_text, examples_block, language)
        else:
